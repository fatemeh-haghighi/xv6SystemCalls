      ...
      for (int j = 0 ; j < NSYS; j++)
      {
        ...
        if(system_calls[i][j].counter > 0)
        {
          ...
          for(int k = 0 ; k < system_calls[i][j].time_index; k++)
          {
            ...
            cprintf("in %d call of this system call\n" , k + 1);
            cprintf("number of calls: %d\n", system_calls[i][j].counter);
            if(system_calls[i][j].value_cnt > 0 || system_calls[i][j].char_cnt > 0)
            {
              cprintf("arg1: %s\t", system_calls[i][j].arg_name[0]);
              if(strncmp(system_calls[i][j].arg_name[0] , "char*" , strlen("char*")) == 0)
                cprintf("char value of arg1 is : %s\n" , system_calls[i][j].arg_char[0][k]);  
              else
                cprintf("value of arg1 is : %d\n" , system_calls[i][j].arg_value[0][k]);  
            }
            if(system_calls[i][j].value_cnt > 0 || system_calls[i][j].char_cnt > 0)
            {
              cprintf("arg2: %s\t", system_calls[i][j].arg_name[1]);
              if(strncmp(system_calls[i][j].arg_name[1] , "char*" , strlen("char*")) == 0)
                cprintf("char value of arg2 is : %s\n" , system_calls[i][j].arg_char[1][k]);  
              else
                cprintf("value of arg2 is : %d\n" , system_calls[i][j].arg_value[1][k]);  

            }
            if(system_calls[i][j].value_cnt > 0 || system_calls[i][j].char_cnt > 0)
            {
              cprintf("arg3: %s\t", system_calls[i][j].arg_name[2]);
              if(strncmp(system_calls[i][j].arg_name[2] , "char*" , strlen("char*")) == 0)
                cprintf("char value of arg3 is : %s\n" , system_calls[i][j].arg_char[2][k]);  
              else
                cprintf("value of arg3 is : %d\n" , system_calls[i][j].arg_value[2][k]);  
